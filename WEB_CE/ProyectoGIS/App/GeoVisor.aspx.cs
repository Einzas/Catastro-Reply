    ...
    public partial class Default : System.Web.UI.Page
    {
        ...
        private void BuildMap()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    lat = Convert.ToDouble(row["latitud"].ToString());
                    lng = Convert.ToDouble(row["longitud"].ToString());
                    GLatLng latLng = new GLatLng(lat, lng);



                    GIcon icon = new GIcon();
                    icon.image = "http://localhost:57049/img/favicon.ico";
                    ...
                    GInfoWindow window = new GInfoWindow(marker,
                                                         string.Format(
                                                             @"<span style='color:blue;'><b>Gasolinera<b> {0} </span><br /><br />
                                                               <span style='color:red;'><b>Longitud y Latitud de Ubicación</b></span><br />
                                                               <span style='color:blue;'><b>Latitutd:</b></span> {1} <br /> 
                                                               <span style='color:blue;'><b>Longitud:</b>:</span> {2} <br />
                                                               <b>Observaciones:</b> {3}",
                                                                row["nombre"].ToString(),
        ...
        protected void btnInsertar_Click(object sender, EventArgs e)
        {
            if(txtNombre.Text == "")
            {
                lblMensaje.Text = "Debe ingresar un Nombre para Insertar";
            }
            else
            {
                lblMensaje.Text = "";
                BD.InsertInTable(txtNombre.Text, txtObservacion.Text, txtLatitud.Text, txtLongitud.Text);
                BuildMap();
                limpiar();
            }
        ...
        {
            BD.UpdateInTable(txtNombre.Text, txtObservacion.Text, txtLatitud.Text, txtLongitud.Text);
            BuildMap();
            limpiar();
        }
        ...
        protected void DropDownList1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                listaPunto.Clear();
                lblTitulo.Text = "Gasolineras Cercanas";
                bool[] habilitar = { true, false, false, false, false, false, false, false };
                VisibleTexto(habilitar);
            }
            ...
            {
                listaPunto.Clear();
                lblTitulo.Text = "Rutas de Distribuición";
            ...
            {
                listaPunto.Clear();
                lblTitulo.Text = "Zonas de Alcance de combustible";
